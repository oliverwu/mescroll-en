...
(function(name, definition) {
	...
	var MeScroll = function(mescrollId, options) {
		...
		me.scrollDom = me.isScrollBody
			? document.body
			: me.getDomById(mescrollId); // MeScroll的滑动区域
	...
	MeScroll.prototype.extendUpScroll = function(optUp) {
		...
		MeScroll.extend(optUp, {
			...
			htmlLoading:
				'<p class="upwarp-progress mescroll-rotate"></p><p class="upwarp-tip">Loading..</p>', // 上拉加载中的布局
	...
	MeScroll.prototype.initDownScroll = function() {
			...
			if (moveY > 0) {
				...
				if (scrollTop <= 0) {
					...
					if (
						...
						!me.optDown.isLock &&
						(!me.isUpScrolling ||
							(me.isUpScrolling && me.optUp.isBoth))
					) {
						...
						if (me.downHight < me.optDown.offset) {
							if (me.movetype !== 1) {
								...
								me.downwarp.classList.remove(
									me.optDown.resetClass
								); // 移除高度重置的动画
								me.isMoveDown = true; // 标记下拉区域高度改变,在touchend重置回来
								if (me.os.ios && !me.isKeepTop) {
									// 下拉过程中,滚动条一直在顶部的,则不必取消回弹,否则会闪白屏
									me.scrollDom.classList.add(
										me.optDown.hardwareClass
									); // 开启硬件加速,解决iOS下拉因隐藏进度条而闪屏的问题
									me.scrollDom.style.webkitOverflowScrolling =
										"auto"; // 取消列表回弹效果,避免与下面me.downwarp.style.height混合,而导致界面抖动闪屏
				...
				if (
					...
					!me.isUpScrolling &&
					(!me.isDownScrolling ||
						(me.isDownScrolling && me.optDown.isBoth)) &&
					(clientHeight + me.optUp.offset >= scrollHeight ||
						toBottom <= 0)
				) {
					me.triggerUpScroll();
				}
		...
		me.touchendEvent = function() {
			...
			if (me.os.pc) {
				me.scrollDom.removeEventListener(
					"mousemove",
					me.touchmoveEvent
				); // 移除pc端的move事件
				document.ondragstart = function() {
		...
		if (me.optDown.use) {
			...
			me.downwarp.innerHTML =
				'<div class="downwarp-content">' +
				me.optDown.htmlContent +
				"</div>";
			var downparent = me.optDown.warpId
				? me.getDomById(me.optDown.warpId)
				: me.scrollDom;
	...
	MeScroll.prototype.endDownScroll = function() {
		...
		var endScroll = function() {
			...
			if (me.downProgressDom)
				me.downProgressDom.classList.remove("mescroll-rotate");
		};
		// 结束下拉刷新时的回调
		...
		me.scrollEvent = function() {
			...
			if (
				...
				if (!me.optUp.isLock && me.optUp.hasNext) {
					var toBottom =
						me.getScrollHeight() - me.getClientHeight() - scrollTop; // 滚动条距离底部的距离
	...
	MeScroll.prototype.bounceTouchmove = function(e) {
		...
		while (el !== document.body && el !== document) {
			...
			if (cls) {
				if (
					cls.contains("mescroll") ||
					cls.contains("mescroll-touch")
				) {
	...
	MeScroll.prototype.hideUpScroll = function(displayAble) {
		...
		var upProgressDom = this.upwarp.getElementsByClassName(
			"upwarp-progress"
		)[0];
	...
	MeScroll.prototype.loadFull = function() {
		...
		if (
			...
			setTimeout(function() {
				...
				if (me.getScrollHeight() <= me.getClientHeight())
					me.triggerUpScroll();
	...
	MeScroll.prototype.showEmpty = function() {
		...
		if (emptyWarp) {
			...
			if (optEmpty.tip)
				str += '<p class="empty-tip">' + optEmpty.tip + "</p>"; // 提示
			if (optEmpty.btntext)
			...
			if (optEmpty.btnClick) {
				// 点击按钮的回调
				var emptyBtn = me.emptyDom.getElementsByClassName(
					"empty-btn"
				)[0];
	...
	MeScroll.prototype.setTopBtnFadeDuration = function(fadeDuration) {
		if (this.toTopBtn) {
			var duration =
				(fadeDuration != null
					? fadeDuration
					: this.optUp.toTop.fadeDuration) + "s";
	...
	MeScroll.prototype.lazyLoad = function(delay) {
		...
		var timer = setTimeout(function() {
			var domArr = me.scrollDom.querySelectorAll(
				"[" + me.optUp.lazyLoad.attr + "]"
			);
			var len = domArr.length;
			for (var i = 0; i < len; i++) {
				var dom = domArr[i];
				if (
					dom.getAttribute(me.lazyTag) !== "true" &&
					me.isInSee(dom, me.optUp.lazyLoad.offset)
				) {
	...
	MeScroll.prototype.isInSee = function(dom, offset) {
		...
		return (
			(topDom < bottomSee && topDom >= topSee) ||
			(bottomDom <= bottomSee && bottomDom > topSee)
		);
	};
	...
	MeScroll.prototype.getDomById = function(id) {
		...
		if (!dom)
			console.error(
				'the element with id as "' +
					id +
					'" can not be found: document.getElementById("' +
					id +
					'")==null'
			);
		return dom;
